...
rm -rf $dist
cargo build -p abi -p cli --release --target x86_64-pc-windows-msvc
MSYS_NO_PATHCONV=1 docker run --rm -t -v "$PWD:/io" -w /io messense/cargo-zigbuild:0.23.3 sh -c "
cargo zigbuild -p abi -p cli --release --target x86_64-unknown-linux-gnu --target aarch64-apple-darwin"
...
cp target/x86_64-pc-windows-msvc/release/cli.exe $dist/dice-windows-x64.exe
cp target/x86_64-unknown-linux-gnu/release/cli $dist/dice-linux-x64
cp target/aarch64-apple-darwin/release/cli $dist/dice-mac-arm
