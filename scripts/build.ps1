...
docker run --rm -t -v $pwd\:/io -w /io messense/cargo-zigbuild:0.23.3 sh -c "
...
cp target/x86_64-pc-windows-msvc/release/abi.dll target/dist/sprite_dicing.dll
cp target/x86_64-unknown-linux-gnu/release/libabi.so target/dist/sprite_dicing.so
cp target/aarch64-apple-darwin/release/libabi.dylib target/dist/sprite_dicing.dylib
